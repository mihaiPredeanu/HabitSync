// app/db/syncJournal.ts
import { getSyncQueue, clearSyncQueue } from './syncQueue';

const API_BASE_URL = 'http://localhost:3000';

export const syncJournalEntries = async () => {
  ...
  if (create.length || update.length || del.length) {
    try {
      await fetch(`${API_BASE_URL}/journal/bulk-upsert`, {
        method: 'POST',
...
export const pullJournalEntriesFromBackend = async (upsertJournalEntry: (entry: any) => Promise<void>) => {
